...
hook.Add( "CanTool", "CRP:Admin:DisableTool", function( _p, tr, _ )

    local _e = tr.Entity;
    if( _e && IsValid( _e ) && _e:IsVehicle() ) then
        return _p:IsInStaff()
    end

end );

hook.Add( "PhysgunPickup", "CRP:Admin:DisablePhysgun", function( _p, _e )
