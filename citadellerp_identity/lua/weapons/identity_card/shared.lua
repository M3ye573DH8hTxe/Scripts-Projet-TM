...
local VUMat = Material("id_card.png")
...
function SWEP:DrawHUD()
	...
	draw.SimpleText(LP:getDarkRPVar("rpname"), "vukein_font", TextW, TextH, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
...
hook.Add("PreDrawTranslucentRenderables", "DrawDICards", function()
	...
	for k,v in pairs(player.GetAll()) do
		...
		if v != LPlayer and IsValid(CurWep) and v:GetActiveWeapon():GetClass() == "identity_card" and v:HasWeapon("identity_card") then
			...
			if boneindex then	
				...
				cam.Start3D2D(HPos, HAng, .05)
					...
					draw.SimpleText(v:getDarkRPVar("rpname"), "vukein_font_bis", TextW, 50, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
