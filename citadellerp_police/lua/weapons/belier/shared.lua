...
local function ramDoor(ply, trace, ent, wep)
    ...
    timer.Simple( 0.4, function() 
        ...
        if( not CitadelleRP.Police.Config.InstantOpen and (not ent:GetNWInt( "CRP_BELIER_DoorHP", false ) or ent:GetNWInt( "CRP_BELIER_DoorHP", 0 ) > 0) ) then
            if( not ent:GetNWInt( "CRP_BELIER_DoorHP", false ) ) then
                ent:SetNWInt( "CRP_BELIER_DoorHP", 100 )
            end
            ...
            local Damage = 15*percent
            ent:SetNWInt( "CRP_BELIER_DoorHP", math.max( ent:GetNWInt( "CRP_BELIER_DoorHP", 0 )-Damage, 0 ) )
        end

        if( CitadelleRP.Police.Config.InstantOpen or ent:GetNWInt( "CRP_BELIER_DoorHP", 0 ) <= 0) then
            ent:keysUnLock()
            ent:Fire( "open", "", 0 )
            ent:Fire( "setanimation", "open", 0 )

            if( not CitadelleRP.Police.Config.InstantOpen ) then
                timer.Create( "CRP_BELIER_DoorTimer_" .. ent:EntIndex(), 60, 1, function()
                    if( IsValid( ent ) ) then
                        ent:SetNWInt( "CRP_BELIER_DoorHP", 100 )
                    end
                end )
...
function SWEP:Think()
    if( CitadelleRP.Police.Config.InstantOpen or not IsValid( self.Owner ) ) then return end
...
if( CLIENT ) then
    ...
    function SWEP:DrawHUD()
        ...
        surface.SetDrawColor( Color( 30, 33, 36 ) )
        surface.DrawRect( (x + sizet/2)+(BarWidth), y + sizet/2, 3, height - sizet )
        
        status = TraceEnt:GetNWInt( "CRP_BELIER_DoorHP", 100 )/100
