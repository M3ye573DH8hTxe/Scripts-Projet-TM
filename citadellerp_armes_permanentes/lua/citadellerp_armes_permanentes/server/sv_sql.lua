...
if ( DATABASE.useMysql ) then
    ...
    if !boolSuccess then
        return print('[CRP - Avantages] - [MYSQL] : Mysql is not installed!')
    end    
...
function DATABASE:Query(strQuery, tbl)
    ...
    function query:onError(err)
        print("CRP - Avantages [MYSQL] : ", err or "")
    end
...
function DATABASE:onConnected()
    local strQuery = [[
        CREATE TABLE IF NOT EXISTS `crp_avantages` (`steamid` VARCHAR(255) NOT NULL UNIQUE, `avantages` TEXT);        
    ]]
...
CitadelleRP.Avantage.SQL = DATABASE